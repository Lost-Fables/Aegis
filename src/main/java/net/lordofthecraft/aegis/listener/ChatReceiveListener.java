	...
	public void receive(PacketReceiveEvent<Chat> event) {
		if (plugin.getDaemon().isAwaitingAuthentication(event.getPlayer())) {
			if (!event.getPacket().getMessage().split(" ")[0].equalsIgnoreCase("/auth")) {
				new ChatBuilder("Error: ").color(RED).append("You are required to authenticated. Use ").color(WHITE).append("/auth [auth code]").color(GOLD).append(" to authenticate").color(WHITE).send(event.getPlayer());
